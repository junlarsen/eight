...
///
/// Because a Span has a size of 8, we allow it to be [`Copy`]. It is now the same cost as passing a
/// reference or pointer around.
...
/// Ensure that the size of a Span is 8 bytes.
const _: () = [(); 1][(size_of::<Span>() == 8) as usize ^ 1];
